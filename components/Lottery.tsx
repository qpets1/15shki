...
const Lottery: React.FC<LotteryProps> = ({ isSpinning, result, prize, hasSpun, onSpin, onPlayAgain, onBuySpin, canAffordSpin }) => {
      ...
      <div className="bg-slate-800 p-8 rounded-2xl shadow-2xl text-center border-2 border-yellow-500 animate-fade-in w-full max-w-sm">
        ...
        <div className="grid grid-cols-3 gap-3 bg-slate-900/50 p-4 rounded-lg mb-6">
          {Array.from({ length: 9 }).map((_, index) => (
            <div key={index} className="w-full aspect-square bg-slate-700 rounded-md flex items-center justify-center text-4xl">
              {isSpinning ? (
